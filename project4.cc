...
positionAlloc1->Add (Vector (95.0, 108.66, 0.0));
positionAlloc1->Add (Vector (105.0, 108.66, 0.0));
positionAlloc1->Add (Vector (90.0, 100.0, 0.0));
positionAlloc1->Add (Vector (110.0, 100.0, 0.0));
positionAlloc1->Add (Vector (95.0, 91.34, 0.0));
positionAlloc1->Add (Vector (105.0, 91.34, 0.0));
...
//EnableAsciiAll()告诉helper你想要将ASCII tracing安装在仿真中的点到点设备上，并且你想要接收端以ASCII格式写出数据包移动信息。
   ...
   phy1.EnableAsciiAll (ascii2.CreateFileStream("phy1.tr")); //包含两个方法调用。
//CreateFileStream()用未命名的对象在协议栈中创建了一个文件流，并把这个文件流传递给了调用方法，即创建了一个对象代表着一个名为“first.tr”的文件，并传递给了NS3。

//EnableAsciiAll()告诉helper你想要将ASCII tracing安装在仿真中的点到点设备上，并且你想要接收端以ASCII格式写出数据包移动信息。
    phy1.EnablePcapAll ("phy1 ");
